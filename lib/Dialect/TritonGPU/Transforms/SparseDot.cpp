...
struct SparseAddEncoding : public OpConversionPattern<SparseDotOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    int num_warps = lookupNumWarps(op);
    int threads_per_warp = lookupNumThreadsPerWarp(op);
    int num_ctas = lookupNumCTAs(op);
...
struct SparseAddEncodingPass
  ...
  void runOnOperation() override {
    // TODO(sparsity): Rewrite this or remove it
    //                 TritonGPUConversionTarget is in the "conversion" part,
    //                 and the Transforms can't depend on Conversion functions
    //                 in OSS builds.
    /*
    ...
    // Create a custom conversion target instead of using TritonGPUConversionTarget
    ConversionTarget target(*context);
    
    // Add the TritonGPU dialect as legal
    target.addLegalDialect<triton::gpu::TritonGPUDialect>();
    
    // Add dynamic legality condition for SparseDotOp
